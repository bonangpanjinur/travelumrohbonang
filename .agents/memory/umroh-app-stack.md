...
Data (packages, site_settings, bookings, etc.) is NOT served from Replit's own Postgres. This note previously claimed that (stale/wrong) — corrected 2026-07-08. In reality: ~90 frontend files call `supabase-js` directly against the real external Supabase Postgres (`VITE_SUPABASE_URL`), and the API server's admin routes use Drizzle over `pg.Pool`. There is only ever meant to be one database — the Supabase one. The local Express shim at `/rest/v1/*` (`artifacts/api-server/src/routes/rest.ts`) exists but most pages bypass it.
