...
Versi: 1.1 | Diperbarui: 2026-07-01 | Status: Living Document
...
└─────────────────────────────────────────────────────────────┘
          ...
          ▼
┌─────────────────┐
│   api-server    │  ← Replit artifact (Express)
│  (health check) │    Hanya health check + proxy ringan
└─────────────────┘
```
...
│   ├── Architecture.md             ← File ini — sumber kebenaran arsitektur
│   ├── Database.md
│   ├── API.md
│   ├── Deployment.md
│   ├── FolderStructure.md
...
Lihat `docs/AuditReport.md` untuk detail current structure dan execution plan refactor.  
...
| Folder | `kebab-case` | `master-data/`, `api-logs/` |
...
└── AuditReport.md        ← Hasil audit & execution plan refactor
```

### Aturan Dokumentasi

1. **Update setiap selesai satu fase refactor** — docs tidak boleh stale
2. **Semua file docs ditulis dalam Bahasa Indonesia** (konsisten dengan standar proyek)
...
6. `docs/Architecture.md` adalah sumber kebenaran utama untuk arsitektur — update di sini, bukan di tempat lain
...
| F05 | Tidak ada `console.log` di production code — gunakan `src/lib/errorLogger.ts` |
...
| F08 | Komponen shadcn/ui di `components/ui/` **tidak boleh** dimodifikasi langsung — extend atau wrap |
...
| 1.1 | 2026-07-01 | Merge dari PROJECT_ARCHITECTURE.md ke docs/Architecture.md; hapus duplikat dari root |
