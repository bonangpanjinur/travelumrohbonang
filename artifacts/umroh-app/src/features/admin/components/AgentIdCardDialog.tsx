...
export default function AgentIdCardDialog({ agent, onOpenChange }: Props) {
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      pdf.setFont("helvetica", "bold"); pdf.setFontSize(7); pdf.text(`ID ${agent.agentCode || "-"}`, CARD_WIDTH / 2, 70.5, { align: "center" }); pdf.setFontSize(6.5); pdf.text(`CABANG ${branchName.toUpperCase().slice(0, 22)}`, CARD_WIDTH / 2, 74.5, { align: "center" });
      if (url) { try { const qr = await QRCode.toDataURL(url, { width: 320, margin: 1, errorCorrectionLevel: "H" }); pdf.addImage(qr, "PNG", 17, 77, 20, 20); } catch { /* QR is omitted when no public page exists */ } }
      pdf.addPage([CARD_WIDTH, CARD_HEIGHT], "portrait"); drawFrame(pdf); pdf.setTextColor(7, 92, 57); pdf.setFont("helvetica", "bold"); pdf.setFontSize(11); pdf.text("DATA AGEN", CARD_WIDTH / 2, 18, { align: "center" }); pdf.setFont("helvetica", "normal"); pdf.setFontSize(6.2); pdf.text(`Kode Referral : ${agent.referralCode || agent.agentCode || "-"}`, 8, 29); pdf.text(`No. MOU       : ${agent.mouNumber || "-"}`, 8, 35); pdf.text(`Bergabung     : ${formatDate(agent.joinedAt)}`, 8, 41); pdf.text(`Berlaku s.d.  : ${formatDate(agent.validUntil)}`, 8, 47); pdf.text(`Cabang        : ${branchCode}${branchName}`.slice(0, 62), 8, 53); pdf.text(`Telepon       : ${agent.phone || "-"}`, 8, 59); pdf.text(`Email         : ${agent.email || "-"}`.slice(0, 58), 8, 65); if (url) { try { const qr = await QRCode.toDataURL(url, { width: 320, margin: 1, errorCorrectionLevel: "H" }); pdf.addImage(qr, "PNG", 17, 69, 20, 20); } catch { /* QR is optional */ } } pdf.setFontSize(5); pdf.text("Scan QR untuk melihat profil agen", CARD_WIDTH / 2, 83, { align: "center" });
