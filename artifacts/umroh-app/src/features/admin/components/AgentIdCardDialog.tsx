...
export default function AgentIdCardDialog({ agent, onOpenChange }: Props) {
  ...
  useEffect(() => {
    if (!agent) return;
    apiFetch<{ data?: Array<{ key: string; value: unknown }> }>("/api/cms/site-settings").then((result) => {
      const setting = result?.data?.find((item) => item.key === "branding");
      if (setting?.value && typeof setting.value === "object") setBranding({ ...defaultBranding, ...(setting.value as Partial<Branding>) });
    }).catch(() => undefined);
  }, [agent]);
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      const addCircularImage = async () => {
        ...
        try {
          const imageUrl = await fetch(agent.photoUrl).then((response) => response.blob()).then((blob) => URL.createObjectURL(blob));
          const image = await new Promise<HTMLImageElement>((resolve, reject) => { const element = new Image(); element.onload = () => resolve(element); element.onerror = reject; element.src = imageUrl; });
      ...
      const addBrandLogo = async () => {
        ...
        try { const image = await fetch(branding.logo_url).then((response) => response.blob()).then((blob) => new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = reject; reader.readAsDataURL(blob); })); pdf.addImage(image, "PNG", 23, 9, 8, 5); return true; } catch { return false; }
      };
      drawFrame(pdf);
      pdf.setTextColor(7, 92, 57); pdf.setFont("helvetica", "bold"); pdf.setFontSize(10); pdf.text(companyName.toUpperCase().slice(0, 23), CARD_WIDTH / 2, 16, { align: "center" }); await addBrandLogo(); pdf.setFontSize(5); pdf.setFont("helvetica", "normal"); pdf.text("TRAVEL & TOURS", CARD_WIDTH / 2, 19.5, { align: "center" });
  ...
  const printCards = async () => {
    ...
    const logo = branding.logo_url ? `<img class="logo" src="${escapeHtml(branding.logo_url)}" alt="${escapeHtml(companyName)}" />` : "";
