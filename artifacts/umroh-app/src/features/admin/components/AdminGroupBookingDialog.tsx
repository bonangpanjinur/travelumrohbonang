...
import {
  Loader2, Plus, Trash2, ChevronRight, Calendar, Package, UserRound,
  Users, CreditCard,
} from "lucide-react";
...
const emptyJamaah = (): JamaahRow => ({
  name: "", phone: "", email: "", gender: "", roomType: "",
});
...
const AdminGroupBookingDialog = ({ open, onOpenChange, onSuccess }: Props) => {
  ...
  useEffect(() => {
    if (!open) return;
    Promise.all([
      apiFetch<{ data: Package[] }>("/api/admin/packages"),
  ...
  // Load departures when package changes
  useEffect(() => {
  ...
  const totalPrice = jamaah.reduce((sum, j) => sum + getPriceForJamaah(j), 0);
  ...
  // Check quota
  const quotaOk = selectedDeparture ? selectedDeparture.remaining_quota >= jamaah.length : true;

  const handleSubmit = async () => {
    if (!canSubmit) return;
    if (!quotaOk) {
      toast({ title: "Kuota tidak cukup", description: `Sisa ${selectedDeparture?.remaining_quota} kursi, dibutuhkan ${jamaah.length} kursi.`, variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
      ...
      toast({
        title: `Booking rombongan berhasil! ${result.bookingCode}`,
        description: `${jamaah.length} jamaah · Total Rp ${totalPrice.toLocaleString("id-ID")}`,
      });
      onSuccess();
      handleClose(false);
    } catch (e: any) {
      toast({ title: "Gagal membuat booking rombongan", description: e?.message, variant: "destructive" });
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex items-center gap-1 mb-4">
          {stepDefs.map((s, i) => (
            <div key={s.n} className="flex items-center gap-1 flex-1">
              ...
              {i < stepDefs.length - 1 && <ChevronRight className="w-3 h-3 text-muted-foreground flex-shrink-0" />}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-4">
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant="secondary" className="text-xs">
                            Sisa {dep.remaining_quota}/{dep.quota}
                          </Badge>
                        </div>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={agentId || "__none__"} onValueChange={(v) => setAgentId(v === "__none__" ? "" : v)}>
                  ...
                  <SelectContent>
                    ...
                    {agents.map((a) => <SelectItem key={a.id} value={a.id}>{a.name} ({a.referral_code})</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
            ...
            <div className="flex justify-end pt-2">
              <Button disabled={!canStep2} onClick={() => setStep(2)} className="gradient-gold text-primary">
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg text-sm text-amber-800 dark:text-amber-300">
              <strong>Pemesan</strong> adalah orang yang memesan (bisa agen, staf, owner, atau kepala rombongan). Boleh berbeda dari jamaah yang berangkat.
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={pemesanPhone} onChange={(e) => setPemesanPhone(e.target.value)} placeholder="08xx..." type="tel" />
              </div>
              <div className="space-y-1.5">
                <Label>Email</Label>
                <Input value={pemesanEmail} onChange={(e) => setPemesanEmail(e.target.value)} placeholder="email@..." type="email" />
              </div>
              <div className="space-y-1.5 col-span-2">
                <Label>Nama Rombongan (opsional)</Label>
                <Input value={groupName} onChange={(e) => setGroupName(e.target.value)} placeholder="cth: Rombongan Masjid Al-Ikhlas" />
              </div>
            </div>

            <div className="flex justify-between pt-2">
              <Button variant="outline" onClick={() => setStep(1)}>← Kembali</Button>
              <Button disabled={!canStep3} onClick={() => setStep(3)} className="gradient-gold text-primary">
                Lanjut <ChevronRight className="w-4 h-4 ml-1" />
              </Button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {!quotaOk && (
              <div className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-sm text-red-700 dark:text-red-300">
                ⚠️ Sisa kuota {selectedDeparture?.remaining_quota} kursi, tapi sudah ada {jamaah.length} jamaah.
                Kurangi jumlah jamaah atau pilih jadwal lain.
              </div>
            )}

            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-muted-foreground">{jamaah.length} jamaah ditambahkan</p>
            ...
            <div className="space-y-3 max-h-[40vh] overflow-y-auto pr-1">
              {jamaah.map((j, idx) => (
                <div key={idx} className="border border-border rounded-lg p-3 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Jamaah {idx + 1}</span>
                    {jamaah.length > 1 && (
                      <button
                        type="button"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <Select value={j.gender} onValueChange={(v) => updateJamaah(idx, "gender", v)}>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {(selectedDeparture?.prices || []).map((p) => (
                          ...
                          >
                            ...
                            <span className="ml-1.5 text-muted-foreground">Rp {p.price.toLocaleString("id-ID")}</span>
                          </button>
                        ))}
                      </div>
                    </div>
            ...
            <div className="flex justify-between pt-2">
              ...
              <Button disabled={!canStep4 || !quotaOk} onClick={() => setStep(4)} className="gradient-gold text-primary">
                Lanjut <ChevronRight className="w-4 h-4 ml-1" />
              </Button>
            </div>
          </div>
        ...
        {step === 4 && (
          <div className="space-y-4">
            <div className="bg-muted/50 rounded-xl p-4 space-y-3">
              <p className="font-semibold text-sm">Ringkasan Booking Rombongan</p>
              <div className="space-y-1.5 text-sm divide-y divide-border/60">
                <div className="pb-2 space-y-1.5">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium capitalize">{paymentScheme === "full" ? "Lunas" : "DP / Cicilan"}</span>
                  </div>
                </div>
                ...
                <div className="pt-2 space-y-1">
                  <p className="text-xs text-muted-foreground font-medium mb-1.5">Jamaah ({jamaah.length} orang):</p>
                  {jamaah.map((j, i) => (
            ...
            <div className="flex justify-between pt-2">
              ...
              <Button
                ...
                className="gradient-gold text-primary min-w-[180px]"
              >
...
export default AdminGroupBookingDialog;
