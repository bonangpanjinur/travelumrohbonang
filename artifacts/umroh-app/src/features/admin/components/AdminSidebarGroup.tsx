...
const AdminSidebarGroup = ({
  ...
  const isGroupActive = items.some((item) => activePathname === item.href || activePathname.startsWith(`${item.href}/`));
    ...
    return (
      <li key={item.href} className="flex items-center">
        <NavLink
          ...
          className={({ isActive }) => `group relative flex min-w-0 flex-1 items-center gap-3 py-2.5 rounded-lg text-sm transition-all ${
        ...
        >
          {({ isActive }) => (
            <>
              <span className={`absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-full bg-accent transition-opacity ${isActive ? "opacity-100" : "opacity-0"}`} />
              <span className={`relative flex items-center justify-center w-7 h-7 rounded-md shrink-0 transition-colors ${isActive ? "bg-accent/20 text-accent" : "text-primary-foreground/50 group-hover:text-primary-foreground/80"}`}>
                <item.icon className="w-4 h-4" />
                {!!badge && <span className="absolute -top-1 -right-1 min-w-[15px] h-[15px] px-0.5 rounded-full bg-accent text-accent-foreground text-[9px] font-bold flex items-center justify-center leading-none">{badge > 9 ? "9+" : badge}</span>}
              </span>
              {!compact && <span className="flex-1 truncate">{highlightMatch(labelFor(item), highlightQuery ?? "")}</span>}
              {!compact && !!badge && <span className="ml-1 shrink-0 min-w-[18px] h-[18px] px-1 rounded-full bg-accent text-accent-foreground text-[10px] font-bold flex items-center justify-center leading-none">{badge > 99 ? "99+" : badge}</span>}
            </>
          )}
        </NavLink>
        {!compact && (
          <button
            ...
            aria-label={isFav ? unfavoriteLabel : favoriteLabel}
            className={`-ml-8 mr-1 z-10 shrink-0 rounded p-1 transition-opacity ${isFav ? "text-accent opacity-100" : "text-primary-foreground/30 opacity-0 group-hover:opacity-100 hover:text-accent"}`}
          >
