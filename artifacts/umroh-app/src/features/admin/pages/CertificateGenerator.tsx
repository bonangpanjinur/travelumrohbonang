          ...
          <div className="space-y-4">
            ...
            <div className="overflow-auto rounded-3xl border bg-slate-200/70 p-4 shadow-inner md:p-8">
              <div className={`certificate-print-root relative mx-auto flex aspect-[1.414/1] max-w-[1000px] flex-col overflow-hidden rounded-xl border-[12px] p-6 text-center shadow-2xl md:p-10 ${design.layout === "modern" ? "bg-white" : design.layout === "classic" ? "bg-[#fffaf0]" : design.layout === "premium" ? "bg-[#fff7ed]" : "bg-[#fffdf7]"}`} style={{ borderColor: design.accent }}>
                ...
                {design.showAdditionalLogo && design.additionalLogoUrl && <img src={design.additionalLogoUrl} alt="Logo tambahan" className="absolute right-6 top-6 h-12 w-12 object-contain md:right-12 md:top-12 md:h-16 md:w-16" />}
                <div className="relative flex flex-col items-center justify-start">
                  {design.showLogo && logoUrl ? <img src={logoUrl} alt="Logo travel" className="mb-2 h-12 w-12 object-contain md:h-16 md:w-16" /> : <div className="mb-2 flex h-12 w-12 items-center justify-center rounded-full md:h-16 md:w-16" style={{ backgroundColor: `${design.accent}22`, color: design.accent }}><Award className="h-6 w-6 md:h-7 md:w-7" /></div>}
                  <p className="text-[10px] font-bold uppercase tracking-[0.25em] md:text-xs md:tracking-[0.3em]" style={{ color: design.accent }}>{companyName}</p>
                  <div className="mx-auto mt-4 h-px w-1/2" style={{ backgroundColor: design.accent }} />
                </div>
                <div className="relative mx-auto flex min-h-0 flex-1 flex-col items-center justify-center px-2 py-4 md:px-8 md:py-6">
                  <h2 className="text-xl font-bold leading-tight tracking-[0.1em] md:text-3xl md:tracking-[0.14em]" style={{ color: design.accent }}>{resolvedTitle}</h2>
                  <p className="mt-4 text-xs text-slate-500 md:mt-5 md:text-sm">{design.subtitle}</p>
                  <p className="mt-2 text-xs text-slate-600 md:mt-3 md:text-sm">{design.body}</p>
