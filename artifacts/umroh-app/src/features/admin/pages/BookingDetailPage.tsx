...
const BookingDetailPage = () => {
  ...
  return (
    <div className="space-y-6 pb-10">
      {/* ── Sticky-feel header ────────────────────────────────────────────── */}
      <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 px-5 py-5 text-white shadow-xl shadow-slate-900/10 sm:px-7 sm:py-6">
        ...
        <nav className="flex items-center gap-1 text-sm text-muted-foreground flex-wrap">
          <Link
            to="/admin/bookings"
            className="text-slate-300 hover:text-white transition-colors flex items-center gap-1 shrink-0"
          >
            Daftar Booking
          </Link>
          <ChevronDown className="w-3.5 h-3.5 -rotate-90 shrink-0 opacity-40" />
          <span className="font-mono text-slate-100 font-semibold shrink-0">{booking.bookingCode}</span>
          {booking.pemesanName && (
            <>
              <span className="opacity-30 shrink-0">·</span>
              <span className="text-slate-300 truncate max-w-[180px]">{booking.pemesanName}</span>
            </>
          )}
        </nav>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              <p className="mb-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-primary-foreground/60">Detail Perjalanan</p>
              <h1 className="font-mono text-2xl font-bold tracking-tight text-white sm:text-3xl">{booking.bookingCode}</h1>
            </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {/* Paket */}
        <div className="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md flex items-start gap-3">
        ...
        {/* Status Bayar */}
        <div className={`group relative overflow-hidden rounded-2xl border bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md flex items-start gap-3 ${booking.paymentStatus === 'paid' ? 'border-green-200' : booking.paymentStatus === 'partial' ? 'border-amber-200' : 'border-red-200'}`}>
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm space-y-5 sm:p-6">
