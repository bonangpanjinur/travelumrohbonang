  ...
  return (
    <main className="min-h-screen bg-[#f7faf8] pb-[calc(8rem+env(safe-area-inset-bottom))]">
      ...
      <div className="mx-auto max-w-5xl space-y-5 px-4 py-5">
        ...
        {activeBooking && <Card className="border-0 shadow-sm"><CardContent className="p-5"><div className="flex items-center justify-between"><div><p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Perjalanan Anda</p><h2 className="mt-1 text-lg font-semibold">Tahapan persiapan</h2></div><CalendarDays className="h-6 w-6 text-primary" /></div><div className="mt-5 grid grid-cols-4 gap-2">{[{ label: "Booking", done: true }, { label: "Pembayaran", done: activeBooking.status === "paid" }, { label: "Dokumen", done: false }, { label: "Berangkat", done: false }].map((step, index) => <div key={step.label} className="relative text-center">{index < 3 && <div className={`absolute left-[calc(50%+1rem)] right-[calc(-50%+1rem)] top-3 h-0.5 ${step.done ? "bg-primary" : "bg-muted"}`} />}<div className={`relative mx-auto flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold ${step.done ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{step.done ? "✓" : index + 1}</div><p className="mt-2 text-[11px] font-medium text-muted-foreground">{step.label}</p></div>)}</div><p className="mt-3 text-xs text-muted-foreground">Lengkapi pembayaran dan dokumen agar status perjalanan Anda siap sebelum keberangkatan.</p></CardContent></Card>}
