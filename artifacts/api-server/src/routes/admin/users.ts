...
router.post("/:id/impersonate", requireSuperAdmin, sensitiveActionLimiter("admin.impersonation", 5), async (req, res) => {
  try {
    if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
      await logSecurityAudit(req, "admin.impersonation", "failure", { entityType: "user", entityId: req.params.id as string, reason: "server_not_configured" });
    ...
    const [target] = await db
      ...
      .limit(1);
    if (!target?.email) {
      await logSecurityAudit(req, "admin.impersonation", "failure", { entityType: "user", entityId: req.params.id as string, reason: "target_not_found" });
    ...
    if (!response.ok) {
      ...
      await logSecurityAudit(req, "admin.impersonation", "failure", { entityType: "user", entityId: req.params.id as string, reason: "provider_error" });
    ...
    if (!actionLink) {
      await logSecurityAudit(req, "admin.impersonation", "failure", { entityType: "user", entityId: req.params.id as string, reason: "missing_action_link" });
    ...
    await logSecurityAudit(req, "admin.impersonation", "success", { entityType: "user", entityId: target.id, target: target.email });
    res.json({ action_link: actionLink });
  } catch (e) {
    console.error("[admin/users] impersonation error", e);
    await logSecurityAudit(req, "admin.impersonation", "failure", { entityType: "user", entityId: req.params.id as string, reason: "unexpected_error" });
