...
import { autoGenerateDepartureChecklists } from "../../lib/checklistCron";
import { expireApprovedBookings } from "../../lib/bookingApprovalExpiryCron";
import { runRetention } from "../../lib/retentionService";
...
// Release seats held by approved bookings that received no payment in time.
router.get("/booking-approval-expiry", async (req, res) => {
  if (!verifyCronSecret(req, res)) return;
  try {
    const result = await expireApprovedBookings();
    res.json({ ok: true, job: "booking-approval-expiry", ...result });
  } catch (err) {
    console.error("[cron] booking-approval-expiry failed:", err);
    res.status(500).json({ error: "Cron job failed" });
  }
});

// ── O-11: Auto-generate pre-departure checklists ─────────────────────────────

router.get("/retention", async (req, res) => {
  if (!verifyCronSecret(req, res)) return;
  try {
    const purgeEnabled = process.env.RETENTION_PURGE_ENABLED === "true";
    const dryRun = !purgeEnabled || req.query.mode !== "purge";
    const result = await runRetention({ dryRun, batchSize: Number(req.query.batchSize) || 500, correlationId: (req as any).correlationId });
    res.json({ ok: true, job: "retention", ...result });
  } catch (err) {
    console.error("[cron] retention failed:", err);
    res.status(500).json({ error: "Retention job failed", correlationId: (req as any).correlationId ?? null });
  }
});

router.get("/checklist-generate", async (req, res) => {
  if (!verifyCronSecret(req, res)) return;
  try {
    const result = await autoGenerateDepartureChecklists();
    res.json({ ok: true, job: "checklist-generate", ...result });
  } catch (err) {
    console.error("[cron] checklist-generate failed:", err);
    res.status(500).json({ error: "Cron job failed" });
  }
});
