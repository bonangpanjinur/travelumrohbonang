...
/** Overall completion across every pilgrim in the booking (mirrors the GET / handler's calc). */
async function computeOverallCompletion(bookingId: string): Promise<number> {
  ...
  const totalVerified = allDocs.filter(
    (d) => d.status === "verified" && (REQUIRED_DOCUMENT_TYPES as readonly string[]).includes(d.documentType),
  ).length;
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const overallCompletionBefore = await computeOverallCompletion(bookingId);
      ...
      // F-03: notify the jamaah once the WHOLE booking's documents just
      // became fully verified (guarded so it only fires on the transition,
      // not on every subsequent PUT once already complete).
      const overallCompletionAfter = await computeOverallCompletion(bookingId);
      if (overallCompletionAfter === 100 && overallCompletionBefore < 100) {
